...
namespace ZWave.Channel.Protocol
{
    internal class Message : IEquatable<Message>
    {
        ...
        public bool Equals(Message other)
        {
            ...
            return Equals(Header, other.Header) && Equals(Type, other.Type) && Equals(Function, other.Function);
        }
        ...
        public static async Task<Message> Read(Stream stream)
        {
            ...
            if (header == FrameHeader.SOF)
            {
                ...
                {
                    if (function == Channel.Function.ApplicationCommandHandler) return new NodeEvent(payload);
                    if (function == Channel.Function.SendData) return new NodeCommandCompleted(payload);
    ...
    internal static class Extensions
    {
        ...
        public static async Task ReadAsyncExact(this Stream stream, byte[] buffer, int offset, int count)
        {
            ...
            var read = 0;
            while (read < count) read += await stream.ReadAsync(buffer, offset + read, count - read);
        }
